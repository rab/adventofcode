...
# --- Day 16: Aunt Sue ---
#
# Your Aunt Sue has given you a wonderful gift, and you'd like to send her a thank you
# card. However, there's a small problem: she signed it "From, Aunt Sue".
#
...
class Sue
  attr_accessor :number, :stuff
  def initialize(number, stuff)
    @number = number
    @stuff = stuff.split(/, */).map {|pair|
      thing, qty = pair.split(/: */)
      [thing,qty.to_i]
    }.to_h
  end

  def match?(gift)
    gift.all? {|thing,qty| !self.stuff.has_key?(thing) || 
               case thing
               when 'cats', 'trees'
                 self.stuff[thing] > qty
...
input.each_line do |line|
  /\ASue (?<number>\d+): (?<stuff>.*)\z/ =~ line.chomp
  suze << Sue.new(number.to_i, stuff)
end
