...
# star 2
encoded_chars = 0
ENCODE = Hash.new {|h,k| h[k] = k}.update('\\' => '\\\\', '"' => '\\"')

input.each_line do |line|
  line = line.strip
  source_chars += line.length
  value = CHARS.match(line)[1]
  value_chars += value.scan(SCAN).size

  encoded = line.gsub(/./, ENCODE)
  encoded_chars += encoded.length + 2
end

puts "%d (code) - %d (value) = %d"%[ source_chars, value_chars, source_chars - value_chars ]
puts "%d (encoded) - %d (code) = %d"%[ encoded_chars, source_chars, encoded_chars - source_chars ]
